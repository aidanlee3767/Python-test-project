...
from simple_agent.agents.news_agent import get_latest_news_json
...
class MainWorkflow:
    ...
    def _execute_function(self, function_name: str, arguments: dict) -> str:
        ...
        try:
            if function_name == "get_time":
                ...
                result = self.controller.get_multiple_times(locations, location_type="city")
                
                if result["status"] == "success":
                    response_lines = []
                    for city, info in result["results"].items():
                        response_lines.append(f"{city}: {info['current_time']} ({info['timezone']})")
                    return "\n".join(response_lines)
                else:
                    return f"오류: {result['message']}"
    ...
    def run(self, user_input: str) -> dict:
        ...
        try:
            ...
            final_response = response.text if response.text else "응답을 생성할 수 없습니다."
